...
def createNewFrameSet(inputMovieName, outputMovieName, firstFrameRepeat):

	if firstFrameRepeat < 1:
	...
	if os.path.isfile(inputCountFileName):
		...
		for i in inputFramePlaySequence:												# loop through all the image files
			...
			if os.path.isfile(inputFileName):
				...
				if i == FIRST_FRAME:	
					for r in range(0, firstFrameRepeat + 1):
						outputFrameSequenceCount = i + r
	...
	success_message = 'new frame set ' + outputMovieName + ' with first frame repeated ' + str(firstFrameRepeat) + ' times successfully created'
...
if __name__ == "__main__":
	...
	parser.add_argument('-r', '--firstframerepeat', dest='firstFrameRepeat', default=FIRST_FRAME_REPEAT, type=int, help='number of times to repeat the first frame on playback')
	...
	debug(progName, 'number of times to repeat the first frame is ' + str(args.firstFrameRepeat))
	debug(progName, 'debug is set to ' + str(printDebugMessages))

	# create the new frame set with the first frame repeated 
	returnCode, returnMessage = createNewFrameSet(args.inputmoviename, args.outputmoviename, args.firstFrameRepeat)
	print returnMessage
